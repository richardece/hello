...
/*
TODO
Something is wrong with client_addr.
Foe every new client connection, different port number is displayed.
But the server adn client can still communicate correctly though.
I would expect that the port number in server side is the same as the port number used by client which is 2000.
Use wireshark to anlyze further.
*/
...
setup_tcp_server_communication(){
    ...
    while(1){
        ...
        printf("Waiting for new client connection or data from existing clients. Blocked on select System call...\n");
        ...
        { 
            ...
            printf("Accept new client connection. Client and Server completes TCP-3 way handshake at this point\n");
        ...
        else /* Data srrives on some other client FD*/
        {
            ...
            for(; i < MAX_CLIENT_SUPPORTED; i++){
                ...
                if(FD_ISSET(monitored_fd_set[i], &readfds)){/*Find the client FD on which Data has arrived*/
